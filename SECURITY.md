...
- **CSP** is `default-src 'self'`. The SvelteKit fallback page inlines a single bootstrap script and Svelte injects inline styles, so the policy must include `style-src 'self' 'unsafe-inline'` and `script-src 'self' 'unsafe-inline'`; no third-party scripts or origins are allowed. The bundled `Caddyfile` sets the same policy.
- **Outbound registry/changelog fetches are SSRF-bounded.** The agent resolves digests only against the registry in the image ref (Docker Hub endpoints are hardcoded) and fetches release notes only from `api.github.com` when an image label points at a GitHub repo. The host is fixed in code, not operator-configurable, so a poisoned label cannot make the agent hit an arbitrary internal URL. Registry and release-feed responses are parsed leniently and never executed; changelog URLs are sanitized before rendering.
